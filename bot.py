...
async def vacancy_must_have(message: Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="🌟 3 кандидата - 2250 Stars", callback_data=f"tariff:stars_2250:{vacancy_id}")],
            [InlineKeyboardButton(text="🚀 15 кандидатов - 7450 Stars", callback_data=f"tariff:stars_7450:{vacancy_id}")],
            [InlineKeyboardButton(text="📧 50+ / интеграция", callback_data=f"tariff:custom:{vacancy_id}")],
        ]
    )
    await message.answer(
        "✅ <b>Вакансия создана!</b>\n\n"
        "Выберите тариф для поиска кандидатов:",
        parse_mode="HTML",
        reply_markup=kb,
    )
...
async def cb_tariff(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.answer(f"❌ Не удалось создать счет: {e}")
        await callback.answer()
...
async def send_payment_invoice(user_id: int, tariff_key: str, vacancy_id: int) -> None:
    ...
    provider_token = getattr(settings, "payments_provider_token", None)
    if not provider_token:
        raise RuntimeError("PAYMENTS_PROVIDER_TOKEN не настроен")
...
async def successful_payment_handler(message: Message):
    ...
    await message.answer(f"✅ Оплачен тариф {tariff['label']}. Ищу до {limit} кандидатов...")
    added = await gather_real_candidates_with_limit(vacancy_id, limit, payment_id)
    await message.answer(
        f"✅ Поиск завершён. Найдено {added} кандидатов.\n\n"
