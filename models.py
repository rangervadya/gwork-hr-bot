...
class Payment(Base):
    ...
    tariff_key: Mapped[str] = mapped_column(String(64))
    candidates_limit: Mapped[int] = mapped_column(Integer)
    candidates_used: Mapped[int] = mapped_column(Integer, default=0)
    status: Mapped[str] = mapped_column(String(32), default=PaymentStatus.CREATED.value, index=True)
    telegram_payload: Mapped[str | None] = mapped_column(String(255), nullable=True)
